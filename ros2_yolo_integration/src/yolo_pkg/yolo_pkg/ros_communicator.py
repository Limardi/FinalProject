...
class RosCommunicator(Node):
    def __init__(self):
        ...
        self.subscriber_dict = {
            "rgb_compress": {
                # The RGB camera publishes here directly (same topic the robot and
                # yolo_example_pkg use). The old "/out/compressed" was produced by a
                # republisher bridge that isn't run in this setup, so the node sat
                # waiting for frames that never arrived.
        ...
        self.publisher_dict = {
            ...
            # Robot navigation reads [detected, dist_m, dx_px] from this topic.
            # detected=1.0 means the target label was seen; dist=-1.0 means no
            # valid depth (treat as "very close / grasp now" in auto_task).
            "yolo_target_info": {
